...
download.file(signal.url, destfile = file.path(out.path, "signal.matrix.txt.gz"))
```
...
R.utils::gunzip(here("data", "signal.matrix.txt.gz"))
l <- stringr::str_split(readLines(here("data", "signal.matrix.txt"), n = 1), "\t", simplify = T)
l <- substr(l, 2, 9)

if (!all(pheno$basename %in% l)){
  stop("Warning: not all pheno samples in the signal matrix")
}
...
```{r icdependence, fig.width=12, fig.height=4}
pheno |> 
...
```{r aucplots, fig.width = 16, fig.height = 4}
library(pROC)
